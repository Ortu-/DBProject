	...
	if(Attenuation<1)
	 {
	  ...
	  float Normal=saturate(dot(Normals,LightVec));
  	  return float4(Diffuse*Normal*Cubelight*LightColor*(1-pow(Attenuation,AttenuationPower))*Fog,1);
         }
	else discard;
     }
	  ...
	  if(ShadowMap>0)
	   {
	    ...
	    LightVec=normalize(-LightVec);
	    float Normal=saturate(dot(Normals,LightVec));
  	    return float4(Diffuse*Normal*Cubelight*LightColor*Attenuation*ShadowMap*Fog,1);
           }
	  else discard;
         }
	else discard;

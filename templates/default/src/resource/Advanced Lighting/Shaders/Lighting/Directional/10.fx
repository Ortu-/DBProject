...
//--------------
  float4 PS_AmbientDirectional(OutPut IN) : COLOR
	...
	return float4(lerp(lerp(Diffuse*((Normal*LightColor)+AmbientLight),Diffuse,floor(Diffuse.w)),Fog,Fog.w),1)*LightPower;
     }
	...
	if(ShadowMap>0)
	 {
	  ...
	  float Normal=saturate(dot(Normals,-LightDirection));
  	  Normal +=pow(saturate(dot(ViewVec,reflect(LightVec,Normals))),Diffuse.w*SpecularPower)*Normals.w*2;
	  Lighting=Normal*LightColor*ShadowMap;
	 }
	//return float4(lerp(lerp(Diffuse*(Lighting+AmbientLight),Diffuse,floor(Diffuse.w)),Fog,Fog.w),1);
	return float4(lerp(lerp(Diffuse*(Lighting+AmbientLight),Diffuse,floor(Diffuse.w)),Fog,Fog.w),1)*LightPower;
     }

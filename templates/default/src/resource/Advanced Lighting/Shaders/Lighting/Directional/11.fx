...
//--------------
  float4 PS_AmbientDirectional(OutPut IN) : COLOR
	...
	float3 AmbientLight=AmbientColor-(tex2D(SSAOSampler,ViewProj).www*AmbientColor);
	...
	float Normal=dot(Normals,-LightDirection);
	float BackLit=pow(1-saturate(-Normal),4*SubSurface.w);
	Normal=saturate(Normal);
	SubSurface.xyz *=(BackLit*pow(1-Normal,2))+(pow(saturate(dot(ViewVec,LightVec)),8)*BackLit);
	Normal +=pow(saturate(dot(ViewVec,reflect(LightVec,Normals))),Diffuse.w*SpecularPower)*Normals.w*2;
	return float4(lerp(lerp(Diffuse*(((Normal+SubSurface)*LightColor)+AmbientLight),Diffuse,floor(Diffuse.w)),Fog,Fog.w),1);
     }
	...
	if(ShadowMap>0)
	 {
	  ...
	  SubSurface.xyz *=(BackLit*pow(1-Normal,2))+(pow(saturate(dot(ViewVec,LightVec)),8)*BackLit);
	  Normal +=pow(saturate(dot(ViewVec,reflect(LightVec,Normals))),Diffuse.w*SpecularPower)*Normals.w*2;
	  Lighting=(Normal+SubSurface)*LightColor*ShadowMap;
	 }

...
//--------------
   OutPut VS(InPut IN)
	...
	OUT.vNormal=normalize(mul(IN.Normal,WorldVP)); 
	OUT.vNormal.y=-OUT.vNormal.y;
...
//--------------
   float4 PS(OutPut IN) : COLOR 
	...
	clip(tex2D(DepthSampler,((IN.Proj.xy/IN.Proj.z)*ViewSize.zw)+ViewSize.xy).y-IN.Proj.w);
        float Angle=dot(IN.Normal,normalize(-IN.ViewPos));
	float4 Diffuse=tex2D(BaseSampler,IN.Tex);
        return float4(lerp(Diffuse.xyz,texCUBE(CubeMapSampler,reflect(-IN.ViewPos,IN.Normal)),ReflectFactor)*lerp(IN.Light1.xyz,IN.Light2,(1-Angle)*LightFactor),(1-Angle)*IN.Light1.w);
    }
   float4 PS_Blocker(OutPut IN) : COLOR 
     {
        float2 ProjUV=IN.Proj.xy/IN.Proj.z;
	clip(tex2D(DepthSampler,(ProjUV*ViewSize.zw)+ViewSize.xy).y-IN.Proj.w);
        float Angle=saturate(dot(IN.Normal,normalize(-IN.ViewPos)));
	float2 Normalmap=tex2D(NormalSampler,IN.Tex);
        return float4(lerp(1-saturate(ProjUV+IN.vNormal*Angle),Normalmap,0.5f),0,Angle*IN.Light1.w);
     }

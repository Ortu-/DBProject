...
//--------------
   Out_AboveWater VS_AboveWater(In_AboveWater IN) 
 	...
 	Out_AboveWater OUT;
	float3 NewPos;
	NewPos.xz=(IN.Pos.xz*(WaterHeight.w/50))+ViewInv[3].xz;
...
//--------------
   float4 PS_AboveWater(Out_AboveWater IN) : COLOR
	...
	float Foam=tex2D(DetailSampler,IN.Tex2.zw+(NormalMap*0.2f)).w*((1-saturate(Depth*0.01f))+pow(NormalMap.w*0.5,8));
	 ...
	 float ViewDis=1-pow(saturate(ViewVecL/WaterHeight.w),2);
	 NormalMap.xyz=lerp(NormalMap.xyz,tex2D(DetailSampler,IN.Tex2+(NormalMap*0.2f))*2,DetailBump*ViewDis);
	#endif
   ...
   float4 PS_UnderWater(Out_UnderWater IN) : COLOR
	...
	#if EnableDetailMap== 1
	 NormalMap=lerp(NormalMap,tex2D(DetailSampler,IN.Tex2+(NormalMap*0.2f))*2,DetailBump);
	#endif

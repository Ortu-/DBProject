	...
	if(Attenuation>0)
	 {
	  ...
	  SubSurface.xyz *=(pow(1-Normal,2)*BackLit)+(pow(saturate(dot(ViewVec,LightVec)),8)*BackLit);
  	  return float4(Diffuse*(Normal+SubSurface)*LightColor*ProjectLight*Attenuation*Fog,1);
         }
	else discard;
     }
	  ...
	  if(ShadowMap>0)
	   {
	    ...
	    Normal=saturate(Normal);
	    SubSurface.xyz *=(pow(1-Normal,2)*BackLit)+(pow(saturate(dot(ViewVec,LightVec)),8)*BackLit);
  	    return float4(Diffuse*(Normal+SubSurface)*LightColor*ProjectLight*Attenuation*ShadowMap*Fog,1);
          }
	  else discard;
         }
	else discard;

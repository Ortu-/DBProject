...
//--------------
   ...
   texture AtlasNormalTexture <string Name = "";>;
   sampler AtlasNormalSampler=sampler_state
      {
 	Texture=<AtlasNormalTexture>;
     	ADDRESSU=Clamp;
      };
...
//--------------
   OutPut VS(InPut IN) 
     {
 	OutPut OUT;
	float ClampUvOffset=TerrainIndex2[IN.Index1.y].y;
	float FlipUvOffset=TerrainIndex2[IN.Index1.y].x;
	float MaxUvOffset=abs(IN.Tex1.y)*TerrainIndex2[IN.Index1.y].z*abs(IN.Tex1.x);
	float2 UvOffset1=float2(0,-1*MaxUvOffset)+(float2(IN.Tex1.y,lerp(IN.Tex1.x,-IN.Tex1.x,FlipUvOffset))*ClampUvOffset);
	float2 UvOffset2=float2(0,1*MaxUvOffset)+(float2(IN.Tex2.y,lerp(IN.Tex2.x,-IN.Tex2.x,FlipUvOffset))*ClampUvOffset);
	float3 NewPos=(IN.Pos.xyz*TerrainSize.z)+TerrainIndex1[IN.Index1.x].xyz;
	NewPos.z=NewPos.z+(abs(UvOffset2.y*TerrainIndex2[IN.Index1.y].w)*(TerrainIndex3[IN.Index3.x].y*TerrainSize.w));
	...
	float LerpHeight=TerrainIndex3[IN.Index3.x]*(1-TerrainIndex2[IN.Index1.y].w);
   ...
   Out_Geometry VS_Geometry(In_Geometry IN) 
	...
	OUT.ViewDis=pow(saturate(length(NewPos-ViewInv[3].xyz)/(TerrainSize.w*30)),3);
	return OUT;
     }
...
//--------------
   ...
   float4 PS_Geometry(Out_Geometry IN) : COLOR
	...
	BlendMap1 *=1-(BlendMap2.y+BlendMap2.z+BlendMap2.w);
	float3 Normals=float3(NormalXZ.x,1-(Vertical.x+Vertical.y)*0.5,NormalXZ.y);
	float3 Tangent=cross(float3(0,0,-1),abs(Normals));
	...
	if(BlendMap>0.99f)
	 {
	  Diffuse=tex2Dlod(Atlas1Sampler,float4(IN.TexU.x,IN.TexV.x,0,MipLevel.x))*BlendMap1.x
          ...
          Normalmap=tex2Dlod(AtlasNormalSampler,float4(IN.TexU.x,IN.TexV.x,0,MipLevel.x))*BlendMap1;
	 }
	else
	 {
	  if(BlendMap>0.01f)
	   ...
	   Normalmap.zw +=tex2Dlod(AtlasNormalSampler,float4(1-IN.TexU.y,IN.TexV.y,0,MipLevel.y)).zw*BlendMap2.zw*Vertical.x;
	   Normalmap.zw +=tex2Dlod(AtlasNormalSampler,float4(1-IN.TexU.z,IN.TexV.z,0,MipLevel.y)).zw*BlendMap2.zw*Vertical.y;
	 }
	Normalmap.xy +=tex2Dlod(AtlasNormalSampler,float4(1-IN.TexU.x,IN.TexV.x,0,MipLevel.x)).xy*BlendMap2.xy;
	Normalmap.xy=float2(Diffuse.w,1-dot(Normalmap,1))*2-1;
        Normalmap.z=1-abs(Normalmap.x+Normalmap.y)*0.25f;
	Normalmap.xyz=mul(Normalmap.xyz,float3x3(Tangent,cross(Tangent,Normals),Normals));
	return PackColor(saturate((Diffuse+ColorMap)-0.5f),float4(saturate(0.5+lerp(Normalmap.xyz,Normals,IN.ViewDis)*0.5),ColorMap.w));
     }
